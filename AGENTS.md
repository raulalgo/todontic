...
- **Markdown round-trip is byte-exact EXCEPT for user-edited bullet regions** (D-011, narrowed by D2 in PRD-01). Precise contract: when the user edits a page's outline, `core/outline.ts` splices a normalized re-serialization (`-` marker, two-space indent) of ONLY the edited bullet region back into the surrounding body verbatim. Unedited foreign files, ALL frontmatter, and all body regions outside the edited outline region are NEVER rewritten. `serializePage` itself is unchanged; all existing byte-exact round-trip tests stay green. Do NOT restore blanket byte-exactness for bullet regions — the normalization on edit is intentional (D2). The #1 project risk (D-011). Any change to ser/de is high-risk; add a round-trip test.
